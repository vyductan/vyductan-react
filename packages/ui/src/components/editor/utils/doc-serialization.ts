...
export async function docToHash(document: SerializedDocument): Promise<string> {
  const cs = new CompressionStream("gzip");
  const writer = cs.writable.getWriter();
  const [, output] = await Promise.all([
    writer
      .write(new TextEncoder().encode(JSON.stringify(document)))
      .then(() => writer.close()),
...
export async function docFromHash(
  ...
  for (let index = 0; index < b64.length; index++) {
    array[index] = b64.codePointAt(index)!;
  }
