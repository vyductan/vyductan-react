...
const isomorphicGetSession = async (headers: Headers) => {
  const authToken = headers.get("Authorization") ?? undefined;
...
export const createTRPCContext = async (options: {
  ...
  const authToken = options.headers.get("Authorization") ?? undefined;
  const session = await isomorphicGetSession(options.headers);
  // const session = {
  //   user: {
  //     id: "user-id-1234",
