...
export const InternalAvatar = (properties: OwnAvatarProps) => {
  const {
    size: customSize,
    src,
    alt,
  ...
  const avatarContext = React.useContext<AvatarContextType>(AvatarContext);

  const size = useSize(
    (contextSize) =>
      customSize ?? avatarContext.size ?? contextSize ?? "default",
  );
