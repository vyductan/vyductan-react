...
import { tagVariants } from "../tag/tag";
...
describe("ConfigProvider component config types", () => {
  test("button config color stays compatible with button variants", () => {
    ...
    type ButtonVariant = ReturnType<typeof useComponentConfig<"button">>["variant"];
    type ButtonVariantProps = NonNullable<Parameters<typeof buttonColorVariants>[0]>;

    expectTypeOf<ButtonColor>().toMatchTypeOf<ButtonVariantProps["color"] | undefined>();
  ...
  test("form config layout stays compatible with form root layout", () => {
    type FormConfigLayout = ReturnType<typeof useComponentConfig<"form">>["layout"];
  ...
  test("tag config variants stay compatible with tag variants", () => {
    ...
    expectTypeOf<TagVariant>().toMatchTypeOf<TagVariantProps["variant"] | undefined>();
    expectTypeOf<TagColor>().toMatchTypeOf<TagVariantProps["color"] | undefined>();
    expectTypeOf<TagSize>().toMatchTypeOf<TagVariantProps["size"] | undefined>();
  });
});
