...
describe("Steps", () => {
  test("infers finish, process, and wait states from current when item status is absent", () => {
    ...
    const titles = [
      ...container.querySelectorAll('[data-slot="steps-item-title"]'),
    ];
    ...
    expect(titles[0]).toHaveClass("text-foreground");
    expect(titles[1]).toHaveClass("text-foreground");
    expect(titles[2]).toHaveClass("text-muted-foreground");
    expect(titles[2]).not.toHaveClass("text-foreground");
  });
  ...
  test("uses one-based numbering in each semantic icon slot when no custom icon is provided", () => {
    ...
    const icons = [
      ...container.querySelectorAll('[data-slot="steps-item-icon"]'),
    ];
  ...
  test("moves the active item title into the icon when vertical title placement has actions", () => {
    const { container } = render(
      <Steps
        current={1}
    ...
    const items = [...container.querySelectorAll('[data-slot="steps-item"]')];
    const activeTitle = items[1]?.querySelector(
      '[data-slot="steps-item-title"]',
    );
    const activePill = within(icons[1] as HTMLElement).getByText("Processing");

    expect(activePill).toHaveClass("px-3", "py-1");
    expect(activeTitle).not.toHaveTextContent("Processing");
    expect(
      within(activeTitle as HTMLElement).getByRole("button", { name: "Done" }),
    ).toBeInTheDocument();
    expect(
      items[1]?.querySelector('[data-slot="steps-item-content"]'),
    ).not.toBeInTheDocument();
  });

  test("keeps the active title visible when actions use a custom item icon", () => {
    const { container } = render(
      <Steps
        current={1}
        titlePlacement="vertical"
    ...
    const items = [...container.querySelectorAll('[data-slot="steps-item"]')];
    const activeIcon = items[1]?.querySelector('[data-slot="steps-item-icon"]');
    const activeTitle = items[1]?.querySelector(
      '[data-slot="steps-item-title"]',
    );
    const activeActions = items[1]?.querySelector(
      '[data-slot="steps-item-actions"]',
    );

    expect(activeIcon).toHaveTextContent("custom");
    expect(activeIcon).not.toHaveTextContent("Processing");
    expect(activeTitle).toHaveTextContent("Processing");
    expect(
      within(activeActions as HTMLElement).getByRole("button", {
        name: "Done",
      }),
    ).toBeInTheDocument();
  });

  test("keeps horizontal action titles before the rail with actions underneath", () => {
    const { container } = render(
      <Steps
        current={1}
        items={[
    ...
    const items = [...container.querySelectorAll('[data-slot="steps-item"]')];
    const activeItem = items[1] as HTMLElement;
    const activeWrapper = activeItem.querySelector(
      '[data-slot="steps-item-wrapper"]',
    );
    const activeIcon = activeItem.querySelector(
      '[data-slot="steps-item-icon"]',
    );
    const activeHeader = activeItem.querySelector(
      '[data-slot="steps-item-header"]',
    );
    const activeSection = activeItem.querySelector(
      '[data-slot="steps-item-section"]',
    );
    const activeRail = activeItem.querySelector(
      '[data-slot="steps-item-rail"]',
    );

    expect(activeWrapper).toHaveClass("w-full");
    expect(activeIcon).not.toHaveTextContent("Processing");
    expect(activeHeader).not.toHaveClass("flex-1");
    expect(activeSection).toHaveTextContent("Processing");
    expect(
      within(activeSection as HTMLElement).getByRole("button", {
        name: "Done",
      }),
    ).toBeInTheDocument();
    expect(activeSection?.compareDocumentPosition(activeRail as Node)).toBe(
      Node.DOCUMENT_POSITION_FOLLOWING,
    );
  });

  test("keeps vertical direction rails in the item header", () => {
    const { container } = render(
      <Steps
        current={0}
        direction="vertical"
    ...
    const header = firstItem?.querySelector('[data-slot="steps-item-header"]');
    const section = firstItem?.querySelector(
      '[data-slot="steps-item-section"]',
    );
    const rail = firstItem?.querySelector('[data-slot="steps-item-rail"]');

    expect(header).toContainElement(rail as HTMLElement);
    expect(section).not.toContainElement(rail as HTMLElement);
    expect(rail?.parentElement).toBe(header);
  });

  test("uses horizontal title placement for actions even in vertical direction", () => {
    const { container } = render(
      <Steps
        current={1}
        direction="vertical"
  ...
  test("applies titlePlacement to every item header and direction to the root as semantic layout hooks", () => {
    ...
    const headers = [
      ...container.querySelectorAll('[data-slot="steps-item-header"]'),
    ];
  ...
  test("keeps horizontal rails in the icon row when title placement is vertical", () => {
    ...
    const rails = [
      ...container.querySelectorAll('[data-slot="steps-item-rail"]'),
    ];
    ...
    expect(firstIcon).toHaveClass("min-h-8", "min-w-8");
    expect(firstIcon).not.toHaveClass("size-8");
    ...
    expect(rails[0]).toHaveClass(
      "absolute",
      "left-[calc(50%+var(--steps-icon-half-width)+0.25rem)]",
      "top-4",
      "h-px",
      "w-[calc(100%-var(--steps-icon-half-width)-var(--steps-next-icon-half-width)-0.5rem)]",
      "bg-primary-600",
    );
    expect(rails[0]).toHaveStyle(
      "--steps-icon-half-width: 16px; --steps-next-icon-half-width: 16px;",
    );
  ...
  test("applies small sizing to icons, titles, content, and vertical placement rails", () => {
    const { container } = render(
      <Steps
        size="small"
        current={1}
        ...
        items={[
          { title: "Scheduled" },
          { title: "Accepted", content: "Guide accepted." },
          { title: "Reminder" },
        ]}
      />,
    );
    ...
    expect(icon).toHaveClass("min-h-6", "min-w-6", "text-xs");
    expect(icon).not.toHaveClass("min-h-8", "min-w-8", "text-sm");
    expect(title).toHaveClass("text-xs");
    expect(content).toHaveClass("mt-1", "text-xs");
    expect(content).not.toHaveClass("text-sm");
    expect(rail).toHaveClass("top-3");
    expect(rail).not.toHaveClass("top-4", "top-7");
    expect(rail).toHaveStyle(
      "--steps-icon-half-width: 12px; --steps-next-icon-half-width: 12px;",
    );
  });

  test("applies large sizing to icons, titles, content, and vertical placement rails", () => {
    const { container } = render(
      <Steps
        size="large"
        current={1}
    ...
    expect(icon).toHaveClass("min-h-14", "min-w-14", "text-xl");
    expect(icon).not.toHaveClass("min-h-8", "min-w-8", "text-sm");
    expect(title).toHaveClass("text-base");
    expect(content).toHaveClass("mt-0", "w-full");
    expect(content).not.toHaveClass("mt-1");
    expect(rail).toHaveClass("top-7");
    expect(rail).not.toHaveClass("top-4");
    expect(rail).toHaveStyle(
      "--steps-icon-half-width: 28px; --steps-next-icon-half-width: 28px;",
    );
  });
