...
function $convertPollElement(domNode: HTMLElement): DOMConversionOutput | null {
  ...
  try {
    parsedOptions = JSON.parse(serializedOptions) as Options;
  } catch {
    return null;
  }

  const node = $createPollNode(question, parsedOptions);
  return { node };
}
...
export class PollNode extends DecoratorNode<JSX.Element> {
  ...
  static importJSON(serializedNode: SerializedPollNode): PollNode {
    ...
    for (const option of serializedNode.options) {
  ...
  static importDOM(): DOMConversionMap | null {
    return {
      span: (domNode: HTMLElement) => {
        if (!Object.hasOwn(domNode.dataset, "lexicalPollQuestion")) {
          return null;
        }
        return {
  ...
  exportDOM(): DOMExportOutput {
    ...
    element.dataset.lexicalPollQuestion = this.__question;
    element.dataset.lexicalPollOptions = JSON.stringify(this.__options);
    return { element };
  }
