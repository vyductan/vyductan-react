...
1. User selects a year (or changes via caption year dropdown) → set sticky preview and mark pending commit
2. If a day is selected → commit immediately and clear pending
3. If popover closes and `commitYearOnClose` is true and pending is set → commit sticky preview
